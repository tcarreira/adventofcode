...
def draw_wire(mapping, crosses, wire, wire_label=1):
    ...
    for direction in wire.split(','):
        ...
        for i in range(steps):
            ...
            if tmp:
                if tmp != wire_label:
                    crosses[str(position)] = wire_label
            mapping[str(position)]=wire_label
...
for cross in crosses:
    cross_int_array = [ int(cross.strip("[]").split(',')[0]), int(cross.strip("[]").split(',')[1]) ]
    distance = abs(cross_int_array[0]) + abs(cross_int_array[1])
