...
def just_sum_array(arr1,arr2):
    return [ arr1[0]+arr2[0], arr1[1]+arr2[1] ]

def draw_wire(mapping, crosses, wire, wire_label=1):
    position=[0,0]
    simple_counter=0
    ...
    for direction in wire.split(','):
        ...
        for i in range(steps):
            simple_counter+=1
            position = just_sum_array(position,vector) # as the initial position will be invalid, just step up first
            tmp = mapping.get(str(position))
            if tmp: # if there is some wire on this coordinates
                if tmp[0] != wire_label:
                    tmp2 = crosses.get(str(position))
                    if tmp2 is None:
                        crosses[str(position)] = mapping[str(position)][1] + simple_counter
            else: # keep only the lowest value
                mapping[str(position)] = (wire_label, simple_counter)


    return (mapping, crosses)

mapping, crosses = draw_wire(mapping, crosses, wire1)
mapping, crosses = draw_wire(mapping, crosses, wire2, wire_label=2)

max_distance = 999e999
for coordinate, distance in crosses.items():
